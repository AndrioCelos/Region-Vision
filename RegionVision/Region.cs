...
public class Region {
	...
	public Region(string name, Rectangle area, bool command = true) {
		...
		var total = 0;
		for (var i = 0; i < name.Length; i++) total += (int) name[i];
		this.colour = (byte) (total % 12 + 13);
	}
	...
	public void SetFakeTiles() {
		...
		if (this.showArea.Top == this.area.Top)
			for (d = 0; d <= this.showArea.Width; d++) this.SetFakeTile(index++, this.showArea.Left + d, this.showArea.Top);
		// East boundary
		if (this.showArea.Right == this.area.Right)
			for (d = 1; d <= this.showArea.Height; d++) this.SetFakeTile(index++, this.showArea.Right, this.showArea.Top + d);
		// West boundary
		if (this.showArea.Width > 0 && this.showArea.Left == this.area.Left)
			for (d = 1; d <= this.showArea.Height; d++) this.SetFakeTile(index++, this.showArea.Left, this.showArea.Top + d);
		// Bottom boundary
		if (this.showArea.Height > 0 && this.showArea.Bottom == this.area.Bottom)
			for (d = 1; d < this.showArea.Width; d++) this.SetFakeTile(index++, this.showArea.Left + d, this.showArea.Bottom);
	}
	...
	public void UnsetFakeTiles() {
		...
		// Top boundary
		if (this.showArea.Top == this.area.Top)
			for (d = 0; d <= this.showArea.Width; d++) this.unsetFakeTile(index++, this.showArea.Left + d, this.showArea.Top);
		// East boundary
		if (this.showArea.Right == this.area.Right)
			for (d = 1; d <= this.showArea.Height; d++) this.unsetFakeTile(index++, this.showArea.Right, this.showArea.Top + d);
		// West boundary
		if (this.showArea.Width > 0 && this.showArea.Left == this.area.Left)
			for (d = 1; d <= this.showArea.Height; d++) this.unsetFakeTile(index++, this.showArea.Left, this.showArea.Top + d);
		// Bottom boundary
		if (this.showArea.Height > 0 && this.showArea.Bottom == this.area.Bottom)
			for (d = 1; d < this.showArea.Width; d++) this.unsetFakeTile(index++, this.showArea.Left + d, this.showArea.Bottom);
